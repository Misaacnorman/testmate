...
export function EditSampleSetDialog({ open, onOpenChange, sampleSet, onSave }: EditSampleSetDialogProps) {
  ...
  const steps = React.useMemo(() => {
    ...
    if (sampleSet.samples.some(s => s.ovenDriedWeight)) {
  ...
  const validateStep = async () => {
    ...
    if (currentStepName === 'Test Results') {
      return await form.trigger(['samples']);
    }
                        ...
                        <div key={field.id} className="p-4 border rounded-lg space-y-4 mb-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Controller name={`samples.${index}.ovenDriedWeight`} control={form.control} render={({ field }) => <Input {...field} type="number" />} />
                                </div>
                                <div className="space-y-2">
                                    <Label>Weight After Soaking (kg)</Label>
                                    <Controller name={`samples.${index}.weightAfterSoaking`} control={form.control} render={({ field }) => <Input {...field} type="number" />} />
                                </div>
                            </div>
                        </div>
                    ))}
