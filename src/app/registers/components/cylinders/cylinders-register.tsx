...
import { getCylinders, deleteCylinderTestGroup, updateSampleSetDetails, updateCylinderTestResults, issueCertificateForCylinderTest } from './data';
import { getCylinderColumns } from './columns';
import { CylinderSample, GroupedCylinderSample } from '@/lib/types';
import { CylindersDataTable } from './data-table';
...
export function CylindersRegister() {
  const [samples, setSamples] = React.useState<GroupedCylinderSample[]>([]);
  ...
  const [selectedSampleSet, setSelectedSampleSet] = React.useState<GroupedCylinderSample | null>(null);
  ...
  const loadSamples = React.useCallback(async () => {
    ...
    try {
      ...
      const groupedData: { [key: string]: CylinderSample[] } = {};
      data.forEach(sample => {
      ...
      const processedData: GroupedCylinderSample[] = Object.values(groupedData).map(group => {
      ...
      console.error('Failed to load cylinder samples:', error);
      toast({
        variant: 'destructive',
        title: 'Error',
 ...
 const handleOpenEditDialog = (sampleSet: GroupedCylinderSample) => {
  ...
  const handleDelete = async (receiptId: string, testId: string, setNumber: number) => {
    try {
      await deleteCylinderTestGroup(receiptId, testId, setNumber);
      toast({
        title: 'Success',
  ...
  const handleSaveEdit = async (data: Partial<GroupedCylinderSample>) => {
    if (!selectedSampleSet || !selectedSampleSet.testId) return;
    try {
  ...
  const handleSaveTestResults = async (data: Partial<GroupedCylinderSample>) => {
    if (!selectedSampleSet || !selectedSampleSet.testId) return;
    try {
      await updateCylinderTestResults(selectedSampleSet.receiptId, selectedSampleSet.testId, selectedSampleSet.setNumber || 0, data);
      toast({
        title: 'Success',
  ...
  const handleIssueCertificate = async (data: any) => {
     if (!selectedSampleSet || !selectedSampleSet.testId) return;
    try {
      await issueCertificateForCylinderTest(selectedSampleSet.receiptId, selectedSampleSet.testId, selectedSampleSet.setNumber || 0, data);
      toast({
        title: 'Success',
  ...
  const columns = React.useMemo(() => getCylinderColumns({ onEdit: handleOpenEditDialog, onDelete: handleDelete }), [handleDelete]);
        ...
        <CardHeader>
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <CardDescription>
                        Detailed records of all cylinder samples.
                    </CardDescription>
                </div>
