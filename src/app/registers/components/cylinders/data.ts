...
import type { CylinderSample, GroupedCylinderSample } from '@/lib/types';
import { fromFirestore } from '@/lib/utils';

const registerCollection = collection(db, 'cylinder-register');

export async function getCylinders(): Promise<CylinderSample[]> {
  ...
  try {
    ...
    if (snapshot.empty) {
        console.log("No documents found in cylinder-register.");
        return [];
    }
    return snapshot.docs.map(doc => fromFirestore<CylinderSample>({ id: doc.id, ...doc.data() }));
  } catch(e) {
    console.error("Error fetching from cylinder-register: ", e);
    // If collection doesn't exist, it will throw. Return empty array in that case.
    return [];
  }
}

export async function updateSampleSetDetails(receiptId: string, testId: string, setNumber: number, data: Partial<GroupedCylinderSample>): Promise<void> {
    if (!testId) {
        throw new Error('testId is required for updating sample set details.');
    }
    ...
    snapshot.docs.forEach(document => {
        ...
        if (individualSampleData) {
            const updatableIndividualFields = ['diameter', 'height', 'weight', 'load', 'modeOfFailure', 'sampleSerialNumber'];
...
export async function deleteCylinderTestGroup(receiptId: string, testId: string, setNumber: number): Promise<void> {
    const q = query(registerCollection, where('receiptId', '==', receiptId), where('testId', '==', testId), where('setNumber', '==', setNumber));
    const snapshot = await getDocs(q);
...
export async function updateCylinderTestResults(receiptId: string, testId: string, setNumber: number, data: Partial<GroupedCylinderSample>): Promise<void> {
    const q = query(registerCollection, where('receiptId', '==', receiptId), where('testId', '==', testId), where('setNumber', '==', setNumber));
    const snapshot = await getDocs(q);
...
export async function issueCertificateForCylinderTest(receiptId: string, testId: string, setNumber: number, data: any): Promise<void> {
    const q = query(registerCollection, where('receiptId', '==', receiptId), where('testId', '==', testId), where('setNumber', '==', setNumber));
    const snapshot = await getDocs(q);
