...
import { GroupedCylinderSample } from '@/lib/types';
...
interface TestResultsDialogProps {
  ...
  sampleSet: GroupedCylinderSample;
  onSave: (data: Partial<GroupedCylinderSample>) => void;
}
  ...
  const processStep = () => {
    if (currentStep < sampleSet.samples.length - 1) {
        ...
        form.setValue(`samples.${currentStep + 1}`, {
            ...
            diameter: currentValues.diameter,
        ...
        const data = form.getValues();
         const finalData: Partial<GroupedCylinderSample> = {
          ...data,
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                        <Controller name={`samples.${currentStep}.diameter`} control={form.control} render={({ field }) => <Input placeholder="Diameter" {...field} type="number" value={field.value ?? ''} />} />
