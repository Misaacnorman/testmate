...
import type { CylinderRegisterEntry, CorrectionFactorMachine, SampleTestResult } from "@/lib/types";
...
export function CylindersTable() {
    const [data, setData] = React.useState<CylinderRegisterEntry[]>([]);
    const [machines, setMachines] = React.useState<CorrectionFactorMachine[]>([]);
    const [loading, setLoading] = React.useState(true);
    const [selectedRow, setSelectedRow] = React.useState<CylinderRegisterEntry | null>(null);
    ...
    const fetchData = React.useCallback(async () => {
        ...
        try {
            const q = query(
              collection(db, "cylinders_register"),
            ...
            const samplesData: CylinderRegisterEntry[] = querySnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data()
            } as CylinderRegisterEntry));
            setData(samplesData);
            const machinesData = machinesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CorrectionFactorMachine));
            setMachines(machinesData);

        } catch (error: any) {
             console.error("Error fetching cylinder register data:", error);
             if (error.code === 'failed-precondition') {
                 setFirestoreError(true);
                  // As a fallback, fetch without sorting to at least show the data
                const fallbackQuery = query(collection(db, "cylinders_register"));
    ...
    const handleSelectRow = (row: CylinderRegisterEntry) => {
    ...
    const openEditDialog = (entry: CylinderRegisterEntry) => {
    ...
    const handleMetadataSubmit = async (updatedData: Partial<CylinderRegisterEntry>) => {
    ...
    const getResultForRow = (row: CylinderRegisterEntry, sampleId: string, field: keyof Omit<SampleTestResult, 'sampleId'>) => {
    ...
    const openDeleteDialog = (entry: CylinderRegisterEntry) => {
    ...
    const handleDeleteEntry = async () => {
        ...
        try {
          await deleteDoc(doc(db, "cylinders_register", deletingEntry.id));
          toast({
            title: "Entry Deleted",
        ...
        <CardHeader>
             <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                    ...
                    <p className="text-muted-foreground text-sm mt-1">Detailed records of all cylinder samples.</p>
                </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {columnVisibility['Dimensions (mm)'] && <TableHead colSpan={2} className="text-center">Dimensions (mm)</TableHead>}
                  ...
                  {columnVisibility['Temperature (°C)'] && <TableHead rowSpan={2}>Recorded<br/>Temperature at<br/>the Facility (°C)</TableHead>}
                  {columnVisibility['Certificate Number'] && <TableHead rowSpan={2}>Certificate<br/>Number</TableHead>}
                  {columnVisibility['Comment'] && <TableHead rowSpan={2}>Comment</TableHead>}
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={26} className="h-24 text-center">
                      Loading cylinder register...
                    </TableCell>
                  </TableRow>
                ) : data.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={26} className="h-24 text-center">
                      No cylinder samples have been registered yet.
                    </TableCell>
                  </TableRow>
                ) : (
                    data.map((row) => {
                    ...
                    return (
                        ...
                        {columnVisibility['Sample ID'] && <TableCell>
                         ...
                         {columnVisibility['Dimensions (mm)'] && <>
                            <TableCell>
                                <div className="flex flex-col">
                                    {sortedSampleIds.map(id => <span key={id}>{getResultForRow(row, id, 'diameter')}</span>)}
                                </div>
                            </TableCell>
                            <TableCell>
                                <div className="flex flex-col">
                        ...
                        {columnVisibility['Certificate Number'] && <TableCell>{row.certificateNumber || '-'}</TableCell>}
