...
const itemSchema = z.object({
  ...
  ovenDriedWeightBeforeSoaking: z.coerce.number(),
  weightAfterSoaking: z.coerce.number(),
  weightOfWater: z.coerce.number(),
  calculatedWaterAbsorption: z.coerce.number(),
...
type EditWaterAbsorptionDialogProps = {
  item: WaterAbsorption;
  onOpenChange: (open: boolean) => void;
  onItemUpdated: (item: WaterAbsorption) => void;
};

export function EditWaterAbsorptionDialog({ item, onOpenChange, onItemUpdated }: EditWaterAbsorptionDialogProps) {
  ...
  return (
    <Dialog open={true} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Edit Water Absorption Test</DialogTitle>
          <DialogDescription>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2"><Label>Oven Dried Weight Before Soaking (kg)</Label><Input type="number" {...form.register("ovenDriedWeightBeforeSoaking")} /></div>
                <div className="space-y-2"><Label>Weight After Soaking (kg)</Label><Input type="number" {...form.register("weightAfterSoaking")} /></div>
              </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2"><Label>Weight of Water (kg)</Label><Input type="number" {...form.register("weightOfWater")} /></div>
                <div className="space-y-2"><Label>Calculated Water Absorption (%)</Label><Input type="number" {...form.register("calculatedWaterAbsorption")} /></div>
              </div>
