...
export const getColumns = (): ColumnDef<Paver>[] => [
  ...
  },
  {
    ...
    header: () => <MultiLineHeader title="Paver Type" />,
  ...
  },
  {
    ...
    header: () => <MultiLineHeader title="Pavers per" subtitle="Square Metre" />,
    cell: ({ row }) => <div>{row.getValue("paversPerSqMetre")}</div>,
  },
    {
    accessorKey: "calculatedArea",
    header: () => <MultiLineHeader title="Calculated" subtitle="Area (mm²)" />,
    cell: ({ row }) => <div>{row.getValue("calculatedArea")}</div>,
  },
  {
