...
type IssuanceWaterAbsorptionsDialogProps = {
  items: WaterAbsorption[];
  onOpenChange: (open: boolean) => void;
  onBatchUpdate: (items: WaterAbsorption[]) => void;
};

export function IssuanceWaterAbsorptionsDialog({ items, onOpenChange, onBatchUpdate }: IssuanceWaterAbsorptionsDialogProps) {
          ...
          <ScrollArea className="flex-grow pr-6 -mr-6">
            <form id="issuance-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="space-y-4 p-2">
                    <div className="space-y-2"><Label>Comment</Label><Input {...form.register('issuance.comment')} /></div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2"><Label>Technician</Label><Input {...form.register('issuance.technician')} /></div>
                        <div className="space-y-2"><Label>Date of Issue</Label><Input type="date" {...form.register('issuance.dateOfIssue')} placeholder="YYYY-MM-DD"/></div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
