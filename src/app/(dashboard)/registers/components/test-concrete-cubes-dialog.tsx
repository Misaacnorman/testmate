...
export function TestConcreteCubesDialog({ cubes, onOpenChange, onBatchUpdate }: TestConcreteCubesDialogProps) {
  ...
  const form = useForm<z.infer<typeof cubeSchema>>({
    ...
    defaultValues: {
      dimensions: {
        length: currentCube.dimensions?.length || 0,
        width: currentCube.dimensions?.width || 0,
        height: currentCube.dimensions?.height || 0,
      },
      weightKg: currentCube.weightKg || 0,
      loadKN: currentCube.loadKN || 0,
      machineUsed: currentCube.machineUsed || '',
      modeOfFailure: currentCube.modeOfFailure || '',
      recordedTemperature: currentCube.recordedTemperature || '',
      certificateNumber: currentCube.certificateNumber || '',
      comment: currentCube.comment || '',
      technician: currentCube.technician || '',
      dateOfIssue: currentCube.dateOfIssue || '',
    },
  });
  ...
  useEffect(() => {
    ...
    form.reset({
      dimensions: {
        length: updatesForThisCube.dimensions?.length ?? cubeToLoad.dimensions?.length,
        width: updatesForThisCube.dimensions?.width ?? cubeToLoad.dimensions?.width,
        height: updatesForThisCube.dimensions?.height ?? cubeToLoad.dimensions?.height,
      },
  ...
  const saveCurrentStep = () => {
    ...
    const hasChanged = JSON.stringify(form.formState.defaultValues) !== JSON.stringify(currentValues);
  ...
  const handleSubmit = () => {
    saveCurrentStep();
    const finalCubesToUpdate = Object.values(updatedCubes).filter(cube => Object.keys(cube).length > 1) as ConcreteCube[];
