...
const formSchema = z.object({
  items: z.array(z.any()), // We don't validate items themselves, just the issuance form
  issuance: issuanceSchema,
});


type IssuanceConcreteCubesDialogProps = {
  items: ConcreteCube[];
  onOpenChange: (open: boolean) => void;
  onBatchUpdate: (items: ConcreteCube[]) => void;
};

export function IssuanceConcreteCubesDialog({ items, onOpenChange, onBatchUpdate }: IssuanceConcreteCubesDialogProps) {
      ...
      <Dialog open={true} onOpenChange={(open) => { if (!open) handleCloseAttempt(); }}>
        ...
        >
          ...
          <ScrollArea className="flex-grow pr-6 -mr-6">
            <form id="issuance-cubes-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <DialogFooter className="pt-4 justify-between">
            ...
            <Button type="submit" form="issuance-cubes-form">Save Issuance Details</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
