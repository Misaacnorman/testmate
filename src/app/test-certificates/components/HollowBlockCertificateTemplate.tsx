...
import { HollowBlockCertificateData } from './HollowBlockCertificateData';
import QRCode from 'qrcode.react';

interface CertificateTemplateProps {
  data: HollowBlockCertificateData;
}

export const HollowBlockCertificateTemplate: React.FC<CertificateTemplateProps> = ({ data }) => {
  const { laboratory } = useAuth();
  const totalHollowArea = data.computationHoles.holeA.total + data.computationHoles.holeB.total + data.computationHoles.notch.total;
    ...
    <div id="certificate-content-printable" className="certificate-container bg-white p-8 font-sans text-[10px] leading-relaxed">
        ...
        <main>
            ...
            <div className="border-2 border-black mb-4">
                <h3 className="text-center font-bold py-1 bg-gray-200 border-b-2 border-black text-sm uppercase">Test Result for Hollow Block</h3>
                <div className="grid grid-cols-2 text-[10px] border-b-2 border-black">
                 ...
                 <div className="grid grid-cols-[150px,1fr] text-[10px] border-b-2 border-black">
                    <div className="p-1 font-semibold border-r-2 border-black">Computation of Hollow Area:</div>
                    <div>
                        <table className="w-full text-center">
                           <thead>
                            <tr className="[&gt;th]:font-semibold [&gt;th]:border-black">
                                <th className="border-l-2">Dimensions of Holes:</th>
                                <th className="border-l-2">L (mm)</th>
                                <th className="border-l-2">W (mm)</th>
                                <th className="border-l-2">No.</th>
                                <th className="border-l-2 border-r-2">Total Hollow Area (mm²)</th>
                                <th>Comment(s)</th>
                            </tr>
                           </thead>
                           <tbody>
                            <tr className="[&gt;td]:border-t-2 [&gt;td]:border-black">
                                <td className="border-l-2 text-left px-1">a</td>
                                <td className="border-l-2">{data.computationHoles.holeA.l}</td>
                                <td className="border-l-2">{data.computationHoles.holeA.w}</td>
                                <td className="border-l-2">{data.computationHoles.holeA.no}</td>
                                <td rowSpan={3} className="border-l-2 border-r-2 align-middle">{totalHollowArea}</td>
                                <td rowSpan={3}></td>
                            </tr>
                             <tr className="[&gt;td]:border-t-2 [&gt;td]:border-black">
                                <td className="border-l-2 text-left px-1">b</td>
                                <td className="border-l-2">{data.computationHoles.holeB.l}</td>
                                <td className="border-l-2">{data.computationHoles.holeB.w}</td>
                                <td className="border-l-2">{data.computationHoles.holeB.no}</td>
                            </tr>
                             <tr className="[&gt;td]:border-t-2 [&gt;td]:border-black">
                                <td className="border-l-2 text-left px-1">notch</td>
                                <td className="border-l-2">{data.computationHoles.notch.l}</td>
                                <td className="border-l-2">{data.computationHoles.notch.w}</td>
                                <td className="border-l-2">{data.computationHoles.notch.no}</td>
                            </tr>
                           </tbody>
                        </table>
                    </div>
                </div>
                <div className="grid grid-cols-[auto,1fr]"><p className="p-1 font-semibold">Compressive Testing Machine ID:</p><p className="p-1">{data.compressiveTestingMachineId}</p></div>
            </div>
            ...
            <table className="w-full border-collapse border-2 border-black text-[10px] text-center mb-4">
                <thead className="bg-gray-200">
                    <tr className="[&>th]:border-2 [&>th]:border-black [&>th]:p-1 [&>th]:font-bold">
                        ...
                        <th rowSpan={2}>EFFECTIVE CROSS<br />SECTIONAL AREA<br />(mm²)</th>
                ...
                <tbody>
                    {data.testResults.map((result, index) => (
                        <tr key={index} className="[&>td]:border-2 [&>td]:border-black [&>td]:p-1">
                            ...
                            <td>{result.effectiveCrossSectionalArea.toFixed(0)}</td>
